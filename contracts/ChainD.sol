...
contract ChainD is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public sellCompanyFee = 1;
    uint256 public sellDonationsFee = 1;
    uint256 public sellMarketingFee = 1;
    uint256 public sellLeadersFee = 1;
    uint256 public sellBurnLPFee = 1;
    uint256 public sellBurnFee = 1;
    uint256 public buyDonationsFee = 1;
    uint256 public buyMarketingFee = 1;
    uint256 public buyLeadersFee = 1;
    uint256 public buyCompanyFee = 1;
    uint256 public buyBurnLPFee = 1;

    uint256 private totalSellFees = 3.0;
    uint256 private totalBuyFees = 3.0;
    ...
    event UpdateFees(
        uint256 sellCompanyFee,
        uint256 sellDonationsFees,
        uint256 sellMarketingFees,
        uint256 sellLeadersFee,
        uint256 sellBurnLPFee,
        uint256 sellBurnFee,
        uint256 buyDonationsFees,
        uint256 buyMarketingFees,
        uint256 buyLeadersFee,
        uint256 buyCompanyFee,
        uint256 buyBurnLPFee
    );
    ...
    constructor() ERC20("ChainD", "CHD") {
        ...
        maxTx = totalTokenSupply / 4; // 25%
        maxWallet = totalTokenSupply / 4; // 25%
    ...
    // exclude a wallet from max transaction amount 
    function setExcludeMaxTx(address account, bool excluded) public onlyOwner {
    ...
    function updateFees(
        uint256 _sellCompanyFee,
        uint256 _sellDonationsFees,
        uint256 _sellMarketingFees,
        uint256 _sellLeadersFee,
        uint256 _sellBurnLPFee,
        uint256 _sellBurnFee,
        uint256 _buyDonationsFees,
        uint256 _buyMarketingFees,
        uint256 _buyLeadersFee,
        uint256 _buyCompanyFee,
        uint256 _buyBurnLPFee
    ) public onlyOwner {
        sellCompanyFee = _sellCompanyFee;
        sellDonationsFee = _sellDonationsFees;
        sellMarketingFee = _sellMarketingFees;
        sellLeadersFee = _sellLeadersFee;
        sellBurnLPFee = _sellBurnLPFee;
        sellBurnFee = _sellBurnFee;
        buyDonationsFee = _buyDonationsFees;
        buyMarketingFee = _buyMarketingFees;
        buyLeadersFee = _buyLeadersFee;
        buyCompanyFee = _buyCompanyFee;
        buyBurnLPFee = _buyBurnLPFee;

        totalSellFees = sellCompanyFee
            .add(sellDonationsFee)
            .add(sellMarketingFee)
            .add(sellLeadersFee)
            .add(sellBurnFee)
            .add(sellBurnLPFee);

        totalBuyFees = buyCompanyFee
            .add(buyDonationsFee)
            .add(buyMarketingFee)
            .add(buyLeadersFee)
            .add(buyBurnLPFee);
    ...
    function _transfer(
        ...
        if (amount == 0) {
            ...
            if (!isBuying && !isSelling) {
                ...
                companyFee = sellCompanyFee;
                donationsFee = sellDonationsFee;
                marketingFee = sellMarketingFee;
                leadersFee = sellLeadersFee;
                burnLPFee = sellBurnLPFee;
                burnFee = sellBurnFee;
                ...
                companyFee = buyCompanyFee;
                donationsFee = buyDonationsFee;
                marketingFee = buyMarketingFee;
                leadersFee = buyLeadersFee;
                burnLPFee = buyBurnLPFee;
                burnFee = 0;
            ...
            uint256 totalFees = companyFee
                .add(donationsFee + marketingFee + leadersFee + burnLPFee + burnFee);
            ...
            if (contractTokenBalance >= swapTokensAtAmount && !automatedMarketMakerPairs[from]) {
                ...
                if (swapAndLiquifyEnabled && burnLPFee > 0 && totalFees > 0) {
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        .mul(buyAmount)
        .div(totalAmount)
        .div(buyCompanyFee + buyMarketingFee + buyLeadersFee);

        dividendsFromSell = address(this)
        .balance
        .mul(sellAmount)
        .div(totalAmount)
        .div(sellCompanyFee + sellMarketingFee + sellLeadersFee);
        ...
        uint256 _completeFees = sellMarketingFee.add(sellCompanyFee).add(sellLeadersFee) +
            buyMarketingFee.add(buyCompanyFee).add(buyLeadersFee);
...
contract ChainDDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        ChainDContract = ChainD(mainContract);
    ...
    function _transfer(
        ...
        require(false, "ChainD_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "ChainD_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ChainD contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            address[] memory path = new address[](2);
            path[0] = uniswapV2Router.WETH();
            path[1] = address(ChainDContract);

            uint256 prevBalance = ChainDContract.balanceOf(address(this));
            ...
            {
                uint256 received = ChainDContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    ChainDContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
